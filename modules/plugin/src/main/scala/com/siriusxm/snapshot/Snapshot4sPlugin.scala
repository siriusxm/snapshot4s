...
object generated {
  implicit val snapshotConfig: SnapshotConfig = new SnapshotConfig(
    resourceDirectory = Path("${snapshot4sResourceDirectory.value.getAbsolutePath.replace("\\","/")}"),
    outputDirectory = Path("${snapshot4sDirectory.value.getAbsolutePath.replace("\\","/")}"),
    sourceDirectory = Path("${sourceBaseDirectory((Test / sourceDirectories).value).getAbsolutePath.replace("\\","/")}")
  )
}
 """
      val snapshot4s = ((Test / sourceManaged).value / "Snapshot4sBuildInfo.scala").getCanonicalFile
